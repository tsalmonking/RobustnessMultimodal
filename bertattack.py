...
def get_important_scores(words, tgt_model, orig_prob, orig_label, orig_probs, tokenizer, batch_size, max_length):
    ...
    all_segs = []
    for text in texts:
        inputs = tokenizer.encode_plus(text, None, add_special_tokens=True, max_length=max_length, truncation=True, padding="max_length")
...
def attack(feature, tgt_model, mlm_model, tokenizer, k, batch_size, max_length=512, cos_mat=None, w2i={}, i2w={}, use_bpe=1, threshold_pred_score=0.3, target_device="cuda:0", mlm_device="cuda:1"):
    ...
    orig_probs = tgt_model(input_ids.unsqueeze(0).to(target_device),
                           attention_mask.unsqueeze(0).to(target_device),
                           token_type_ids.unsqueeze(0).to(target_device)
                           )[0].squeeze()
    ...
    word_predictions = mlm_model(input_ids_.to(mlm_device))[0].squeeze()  # seq-len(sub) vocab
    ...
    for top_index in list_of_index:
        ...
        for substitute_ in substitutes:
            ...
            inputs = tokenizer.encode_plus(temp_text, None, add_special_tokens=True, max_length=max_length, truncation=True, padding="max_length")
            input_ids = torch.tensor(inputs["input_ids"]).unsqueeze(0).to(target_device)
